...
SHA224Hash = input("Please input the hash to crack.\n>")
HashedSHA = hashlib.sha224(bytes(SHA224Hash, 'utf-8')).hexdigest()
...
if inp1 == "y":
    with open("rockyou.txt", "r") as a:
        for line in a:
            line = line.rstrip("\n")
            b = hashlib.sha224((line).encode('utf-8'))
            print("\n")
            print(b.hexdigest())
            
            if (b.hexdigest()) == SHA224Hash:
